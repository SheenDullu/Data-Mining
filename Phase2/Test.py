...
def save(filename):
    ...
    test_data = pd.read_csv(filename, names=column_names)
    features_test = extract_features_test(test_data)
    file_handler = open("models/scaler.pkl", 'rb')
    scaler = pickle.load(file_handler)
    features_test = scaler.transform(features_test)
    for model_name in cls_names:
        file_handler = open("models/" + model_name + ".model", 'rb')
        model = pickle.load(file_handler)
        pd.DataFrame(model.predict(features_test)).to_csv(model_name + '_output.csv', index=False, header=False)
    print('Successfully generated output files for each model')
